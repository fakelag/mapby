...
describe('mapBy.bench', () => {
    ...
    const printSnapshot = (snapshot: LoopLagSnapshot) => {
        console.log(`Finished with delay avg=${toMs(snapshot.delAvgMs)}, max=${toMs(snapshot.delMaxMs)}, min=${
            toMs(snapshot.delMinMs)} loopCount=${snapshot.loopCount} totalTime=${toMs(snapshot.totalTimeMs)}`);
    };

    it('does not block event loop when mapping with a synchronous predicate', async () => {
    ...
    it('blocks event loop with a synchronous predicate when using noDefer', async () => {
        const inp = input1_000_000;

        const { returnValue, exception, snapshot } = await during(
            () => mapBy(inp, { concurrency: 100, noDefer: true }, async item => item*2),
        );
        ...
        expect(snapshot.loopCount).toBeLessThanOrEqual(1);
