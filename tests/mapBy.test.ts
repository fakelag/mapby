...
describe('mapBy', () => {
  ...
  it.each([1, 2, 5, 10, 20])(
    'maps a set of inputs to outputs (concurrency %i)',
    async (concurrency) => {
  ...
  it.each([1, 2, 5, 10, 20])(
    'aborts mapping on error when using abortOnError (concurrency %i)',
    async (concurrency) => {
      const inp = input10;
      ...
      expect(numItemsProcessed).toBeLessThanOrEqual(
        Math.max(errorAtIndex + 1, concurrency),
      );
      ...
      await expect(
        mapBy(inp, { concurrency, abortOnError: true }, async (item, index) => {
          if (index === errorAtIndex) throw errToThrow;
          return `${item * 2}-${index}`;
        }),
      ).rejects.toThrow(errToThrow);
  ...
  it.each([1, 2, 5, 10, 20])(
    'keeps mapping and returns undefined results for errored callbacks (concurrency %i)',
    async (concurrency) => {
      const inp = input10;
      ...
      inp.forEach((v, i) => {
        expect(result[i]).toBe(
          errorAtIndex.includes(i) ? undefined : `${v * 2}-${i}`,
        );
      });
  ...
  it.each([
    ...
    'maps inputs to outputs with retries (concurrency %i, retries %i)',
    async (concurrency, retries) => {
      const inp = input10;
      const errorAtIndex = 3;
